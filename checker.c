...
/*   Updated: 2024/07/11 01:15:51 by csouita          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	is_it_sorted(t_list *stack_a, t_list *stack_b)
{
	...
	while (tmp && tmp->next)
	{
		if (tmp->content > tmp->next->content)
			(ft_putstr_fd("KO\n", 2), free_stacks(&stack_a),
				free_stacks(&stack_b), exit(1));
		tmp = tmp->next;
	}
	if (ft_lstsize(stack_a) == 0)
		(ft_putstr_fd("KO\n", 2), free_stacks(&stack_a), free_stacks(&stack_b),
			exit(1));
	else if (ft_lstsize(stack_b) > 0)
		(ft_putstr_fd("KO\n", 2), free_stacks(&stack_a), free_stacks(&stack_b),
			exit(1));
	else
		(ft_putstr_fd("OK\n", 1), free_stacks(&stack_a), free_stacks(&stack_b));
}
...
void	the_move(char *line, t_list **stack_a, t_list **stack_b)
{
	if (ft_strcmp(line, "sa\n") == 0)
		...
		(free_stacks(stack_a), free_stacks(stack_b), free(line),
			ft_putstr_fd("Error\n", 1), exit(1));
}
